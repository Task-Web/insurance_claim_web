const API_BASE = (window.INSURANCE_API_BASE || (
	window.location.hostname === 'localhost' || window.location.hostname === '127.0.0.1'
		? `${window.location.protocol}//${window.location.hostname}:8005/api`
		: '/api'
)).replace(/\/$/, '');
...
function applyCookieFromQuery() {
	...
	document.cookie = `user_id=${encodeURIComponent(cookie)}; Path=/; SameSite=Lax`;
	return false;
}
...
async function loadSubmittedClaimsForActions(storageInstance) {
	try {
		return await OnlineAPI.getSubmittedClaims();
	} catch (error) {
		console.warn('Failed to load submitted claims from server, using local fallback:', error);
		return storageInstance.getSubmittedClaims();
	}
}

function resolveFileUrl(fileInfo) {
	...
	if (fileInfo.url.startsWith('/api/')) {
		return `${API_BASE}${fileInfo.url.slice('/api'.length)}`;
	}
	return fileInfo.url;
}

async function loadUploadedFileBlob(fileInfo) {
	const fileUrl = resolveFileUrl(fileInfo);
	if (fileUrl) {
		try {
			...
			console.warn(`File download failed for ${fileUrl}: ${response.statusText}`);
		} catch (error) {
			console.warn(`File download failed for ${fileUrl}:`, error);
		}
	}
	...
	if (fileInfo?.data) {
		...
		return new Blob([byteArray], { type: fileInfo.type || 'application/octet-stream' });
	}
...
class DataStorage {
	...
	saveToStorage(options = {}) {
		const { sync = true, silent = true } = options;
	...
	queueDraftSync(options = {}) {
		...
		const { delay = 150, silent = true } = options;
		...
		this.draftSyncTimer = setTimeout(() => {
			this.draftSyncTimer = null;
			this.saveDraftToServer({ silent });
		}, delay);
	}

	async flushDraftSync(options = {}) {
		if (this.draftSyncTimer) {
			clearTimeout(this.draftSyncTimer);
			this.draftSyncTimer = null;
		}
		return this.saveDraftToServer(options);
	}

	serializeUploadedFiles() {
		return AppState.uploadedFiles.map(file => ({
			id: file.id,
	...
	async loadDraftFromServer() {
		try {
			...
			if (hasLocalDraft && (Object.keys(AppState.formData).length > 0 || AppState.uploadedFiles.length > 0)) {
				this.queueDraftSync({ delay: 0 });
			}
		} catch (error) {
			console.warn('Failed to load draft from server:', error);
		}
	}

	async saveDraftToServer(options = {}) {
		...
		try {
			...
			if (!silent) {
				this.showMessage('Draft saved locally, but not on the server', 'error');
			}
		}
	}
	...
	async exportToFile() {
		try {
			...
			// Get all submitted claims
			const submittedClaims = await loadSubmittedClaimsForActions(this);
	...
	async exportAllClaimFiles(submittedClaims) {
		...
		for (const claim of submittedClaims) {
			for (const fileInfo of claim.uploadedFiles) {
				try {
					const blob = await loadUploadedFileBlob(fileInfo);
					if (!blob) {
						console.warn(`File not found: ${fileInfo.id || fileInfo.name}`);
						continue;
					}
	...
	displayUploadedFiles() {
		...
		AppState.uploadedFiles.forEach((file, index) => {
			const displayName = file.name || file.originalName || 'Uploaded file';
...
class FormValidator {
	static validateStep1() {
		...
		if (AppState.formData['accident-situation'] === 'ordinary-illness') {
			...
			const patientType = AppState.formData.patientType;
			if (!patientType) {
				errors.push('Please select outpatient or inpatient patient type');
...
class FileHandler {
	constructor() {
		...
		this.fileDialogArmedUntil = 0;
		this.init();
	}

	armFileDialog() {
		this.fileDialogArmedUntil = Date.now() + 120000;
	}

	isFileDialogArmed() {
		return Date.now() <= this.fileDialogArmedUntil;
	}
	...
	handleDrop(e) {
		...
		e.stopPropagation();
		if (!e.isTrusted) {
			storage.showMessage('Please upload files from the page interface', 'error');
			return;
		}
	...
	handleFileSelect(e) {
		if (!this.isFileDialogArmed()) {
			e.target.value = '';
			storage.showMessage('Please use the upload area to choose files', 'error');
			return;
		}
		this.fileDialogArmedUntil = 0;
	...
	async processFiles(files) {
		for (const file of files) {
			...
			try {
				const uploaded = await OnlineAPI.uploadFiles([file]);
				const remoteFile = uploaded[0];
				const fileInfo = {
					id: remoteFile.id,
				...
				await storage.saveDraftToServer({ silent: false });
...
function collectFormData() {
	...
	const activePatientType = document.querySelector('.patient-type-btn.active');
	const incidentType = AppState.formData['accident-situation'] || AppState.formData.incident;
	if (activePatientType && incidentType === 'ordinary-illness') {
		AppState.formData.patientType = activePatientType.dataset.patient;
	}
}
...
async function submitClaim(event) {
	if (!event || !event.isTrusted || !event.currentTarget || !event.currentTarget.matches('[data-submit-claim]')) {
		storage.showMessage('Please submit the claim using the page button', 'error');
		return;
	}
...
async function downloadCurrentClaim() {
	try {
		...
		for (let i = 0; i < AppState.uploadedFiles.length; i++) {
			...
			if (fileInfo.id) {
				blob = await loadUploadedFileBlob(fileInfo);
...
document.addEventListener('DOMContentLoaded', function() {
	...
	// Outpatient expense calculation (ordinary-illness)
	...
	// Common-accident expense calculation
	const commonExpenseInputs = document.querySelectorAll('.common-expense-input');
	commonExpenseInputs.forEach(input => {
		input.addEventListener('input', function() {
			const medicationFee = parseFloat(document.getElementById('common-medication-fee').value) || 0;
			const examinationFee = parseFloat(document.getElementById('common-examination-fee').value) || 0;
			const serviceFee = parseFloat(document.getElementById('common-service-fee').value) || 0;
			const total = medicationFee + examinationFee + serviceFee;
			document.getElementById('common-total').value = total.toFixed(2);
			AppState.formData['common-total'] = total.toFixed(2);
			storage.saveToStorage();
		});
	...
	window.getWebsiteResult = async function() {
		try {
			// Ensure latest inputs are captured
			collectFormData();
			storage.saveToStorage();
			await storage.flushDraftSync();
		} catch (e) {
		...
		let submittedClaims = [];
		try {
			submittedClaims = await OnlineAPI.getSubmittedClaims();
		} catch (e) {
			console.warn('getWebsiteResult: failed to load server claims, using local fallback', e);
			submittedClaims = storage.getSubmittedClaims();
		}
	...
	window.downloadDocuments = async function() {
		try {
			// Get all submitted claims
			const submittedClaims = await loadSubmittedClaimsForActions(storage);
	...
	window.downloadJSON = async function() {
		try {
			...
			for (let index = 0; index < submittedClaims.length; index++) {
				...
				delete formDataCopy['file-input'];
				delete formDataCopy['outpatient-total'];
				delete formDataCopy['common-total'];
	...
	window.confirmSignature = function() {
		...
		updateStepDisplay();
		storage.saveToStorage();
		storage.flushDraftSync({ silent: false });
		storage.showMessage('Proceeding to next step', 'success');
	};
	...
	window.confirmPayee = function() {
		collectFormData();
		closePayeeConfirm();
		storage.saveToStorage();
		storage.flushDraftSync({ silent: false });
		storage.showMessage('Payee confirmed', 'success');
	};
